...
def meta_test(val_loader, model, epoch_index, best_prec1, opt, train='val'):
    ...
    for episode_index, (query_images, query_targets, query_modal, support_images, support_targets, _) in tqdm(enumerate(val_loader)):
        ...
        # self.
        # support_grid = torchvision.utils.make_grid(torch.reshape(support_images, [-1, 3, 84, 84]), normalize=True, nrow=5)
        # writer.add_image('img/support', support_grid, global_step=episode_index)
        # query_grid = torchvision.utils.make_grid(torch.reshape(query_images, [-1, 3, 84, 84]), normalize=True, nrow=opt.query_num)
        # writer.add_image('img/query', query_grid, global_step=episode_index)

        # for i in range(opt.way_num):
        #     temp_support = support_images[i].repeat(torch.cuda.device_count(), 1, 1, 1)
        #     temp_support = temp_support.type(torch.FloatTensor)
    ...
    for episode_index, (query_images, query_targets, query_modal, support_images, support_targets,) in tqdm(enumerate(val_loader)):
        ...
        for epoch in range(100):
            ...
            for i in range(0, support_size, batch_size):
                ...
                loss.backward()
                set_optimizer.step()
        # scores = linear_module(emb_q)

        # set_optimizer = torch.optim.SGD(linear_module.parameters(), lr = 0.01, momentum=0.9, dampening=0.9, weight_decay=0.001)
        # for ite in range(100):
        #     print(support_targets.size())
        #     pred = linear_module(emb_s)
        #     loss = loss_func(pred, support_targets)
        #     print(loss)
        #     set_optimizer.zero_grad()
        #     loss.backward()
        #     set_optimizer.step()
